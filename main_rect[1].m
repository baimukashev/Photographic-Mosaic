function mosaic_image = main_rect(path_to_dataset,a,b,original_image );
tic
...
for py = 1:pn
    for px = 1:pm
        ...
        for i = 1:p(1)
            for j=1:p(2)
                intensityR = intensityR + double(img_res(i+(py-1)*p(1),j+(px-1)*p(2),1));
                intensityG = intensityG + double(img_res(i+(py-1)*p(1),j+(px-1)*p(2),2));
                intensityB = intensityB + double(img_res(i+(py-1)*p(1),j+(px-1)*p(2),3));
            end
        end
        % find mean 
        ...
        for i = 1:p(1)
            for j=1:p(2)
                for r = 1:3
                    img_res(i+(py-1)*p(1),j+(px-1)*p(2), r) = new_tile(i,j,r);
                end
            end
        end
    end         
